...
https在http的基础上加入了SSL协议，SSL依靠证书来验证服务器的身份。（可以防止中间人攻击）
...
Keychain
UIPasteboard
UIDocumentInteractionController（接收的app需要实现：
1.UIFileSharingEnabled 设置为YES
2. 设置可接收的文件类型 docx doc ref text ppt excel word pdf 
3. plist 修改
4. application:openURL:sourceApplication:annotation:中处理文件
）
5.local socket
app1进行tcp的bind和listen
另一个app在同一个端口进行connect
优点是只要连接保持，随时可以传任何想传的数据 而且带宽足够大
缺点是通信的另外一份需要有在后台运行的权限
场景：比如定位
手机上很多app都接入了同一个公司的SDK

第一次请求时SDK 先尝试local socket连接规定的端口，如果没有的话，自己作为服务器，如果有的话，作为客户端直接去连 如果断开的话，重新连接

app推到后台时，线程挂起，返回前台时，任务恢复

NSTimer和GCDTimer的区别：
iOS上的各种crash的常见原因总结
复杂的多线程case
常见的锁
拦截到unRecognized错误之后，如何处理 
AFN的优势
app 死循环 栈和堆
http的自定义header、cookie
多target开发
NO。1044
读写锁
字符串排列组合
...
VIPER：
  ...
  更新缓存的四种模式
Cache Aside Pattern：  
取数据：先到缓存中取数据，有的话直接返回；没有则去数据库中取数据，然后把数据放到缓存中。
更新数据：先把数据存到数据库中，成功后，再让缓存失效
  
Read/Write Through Pattern
Read Through：在查询操作的时候更新缓存，缓存失效时（过期或者LRU换出），缓存服务自己来加载，调用者只需要获取数据，不需要关心数据的管理
Write Through：在更新操作的时候，如果在缓存中查询到，则更新缓存，然后再自己更新数据库；如果没有查询到，则直接更新数据库

Write Behind Caching Pattern：
更新数据的时候，只更新缓存，不更新数据库，缓存会异步地批量更新数据库（好处：让数据的I/O操作飞快无比；坏处是：可能会带来数据丢失）
